...
ゲームパッケージ版中古ソフトとゲーム機本体の参考相場、買い時・売り時、価格推移を掲載する静的サイトです。現在はSwitchソフトの月次相場と、Switch 2・Switch有機EL・PS5本体の型番別中古価格を公開しています。
...
本体モデルは `data/hardware-catalog.csv`、確認価格は `data/hardware-observations.csv` で管理します。型番・言語仕様・ドライブ有無を固定し、欠品・状態難・ランクBを除いた通常中古品だけを記録します。`npm run build` により `data/hardware-summary.json` を生成します。3店舗未満のモデルはサイト上で「参考度：低」と表示します。
